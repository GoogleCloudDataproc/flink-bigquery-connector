 ...
 *
 * <p><b>Why this extends {@code ParquetWriterFactory} instead of returning one directly:</b> {@code
 * BigQuerySinkConfig.equals} compares the bulk-writer factory by value, and Flink's {@code
 * ParquetWriterFactory} inherits identity-based equality. Holding the {@link RowType} as a
 * serializable field and overriding {@code equals}/{@code hashCode} keeps equality stable across
 * {@code BigQueryDynamicTableSink.copy()} round-trips.
 *
...
public final class RowDataParquetWriterFactory extends ParquetWriterFactory<RowData> {
    ...
    private static Configuration buildConf() {
        ...
        conf.set("parquet.compression", "ZSTD");
        return conf;
    }
