...
**Note on unaligned checkpoints:** Flink disables unaligned checkpoints for sink post-commit edges as a policy, so this design doesn't have to reason about UC interactions with checkpoint barriers at the post-commit operator.
...
3. `flink-parquet` + BigQuery load jobs support microsecond-precision `TIMESTAMP` out of the box
   - In comparison, `flink-avro` today requires either OSS changes or a hacky/unoptimized workaround to get microsecond-precision `TIMESTAMP`. Nothing prevents us from adding Avro as a follow-up — it covers a wider range of BigQuery types in theory.
