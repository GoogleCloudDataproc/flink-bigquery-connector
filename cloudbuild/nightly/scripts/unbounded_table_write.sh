...
# Copy the table
NEW_SOURCE_TABLE_NAME="$SOURCE_TABLE_NAME"-"$timestamp"
if [ "$IS_SQL" == True ]
then
  NEW_SOURCE_TABLE_NAME="$NEW_SOURCE_TABLE_NAME"-SQL
fi
if [ "$IS_EXACTLY_ONCE_ENABLED" == True ]
then
  NEW_SOURCE_TABLE_NAME="$NEW_SOURCE_TABLE_NAME"-EXO
else
  NEW_SOURCE_TABLE_NAME="$NEW_SOURCE_TABLE_NAME"-ALO
fi
bq cp -f "$DATASET_NAME"."$SOURCE_TABLE_NAME" "$DATASET_NAME"."$NEW_SOURCE_TABLE_NAME"
...
DESTINATION_TABLE_NAME="$SOURCE_TABLE_NAME"-sink
