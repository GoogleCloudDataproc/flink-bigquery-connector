...
def get_bq_query_rows(client_project_name,project_name, dataset_name, query):
  client = bigquery.Client(project=client_project_name)
  temp_table_name = 'temp_query_table'
  table_id = f'{project_name}.{dataset_name}.{temp_table_name}'
  job_config = bigquery.QueryJobConfig(destination=table_id, write_disposition='WRITE_TRUNCATE')
  query_job = client.query(query, job_config)
  query_job.result()
  # TODO: Delete the temp table created.
  return get_bq_table_rows(client_project_name, project_name, dataset_name, temp_table_name, '')
...
def get_bq_table_rows(client_project_name, project_name, dataset_name, table_name, query):
  if query:
    return get_bq_query_rows(client_project_name, project_name, dataset_name, query)
...
def get_blob_and_check_metric(
  gcs_log_object,cluster_temp_bucket, metric_string, metric_delimiter
):
  logs_as_string = ''
  try:
...
def read_logs(cluster_temp_bucket, logs_pattern):
...
def run(
  ...
  metric = read_logs(cluster_temp_bucket, logs_pattern)
