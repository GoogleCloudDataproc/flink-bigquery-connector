...
python3 cloudbuild/python/nightly_job/create_partitioned_table.py -- --now_timestamp="$now" --project_name="$PROJECT_NAME" --dataset_name="$DATASET_NAME" --table_name="$TABLE_NAME"
...
python3 cloudbuild/python/nightly_job/insert_dynamic_partitions.py -- --now_timestamp="$now" --project_name="$PROJECT_NAME" --dataset_name="$DATASET_NAME" --table_name="$TABLE_NAME" --refresh_interval="$PARTITION_DISCOVERY_INTERVAL"
