...
public class BigQueryDynamicTableFactoryTest {
	...
	public void requiredOptionsTestSuccess() {
		...
		BigQueryDynamicTableFactory bigQueryDynamicTableFactory = new BigQueryDynamicTableFactory();
		List<String> expectedOptions = Arrays.asList("materializationDataset", "materializationProject", "query",
		...
		List<String> options = configOptionSet.stream().map(configOption -> configOption.key().toString())
	...
	public void optionalOptionsTestSuccess() {
		List<String> expectedOptions = Arrays.asList("arrowCompressionCodec", "bqBackgroundThreadsPerStream",
				"bqEncodedCreateReadSessionRequest", "credentials", "credentialsFile", "defaultParallelism", "filter",
				"format", "gcpAccessToken", "maxParallelism", "parallelism","parentProject", "partitionExpirationMs", "partitionField",
				"partitionRequireFilter", "partitionType", "proxyPassword", "proxyUri", "proxyUsername");
	...
	public void createDynamicTableSourceTestSuccess() {
		...
		FactoryUtil.TableFactoryHelper inputHelperObject = FactoryUtil
				.createTableFactoryHelper(bigQueryDynamicTableFactory, MockDynamicTableContext);
		...
		assertThat(options.get(ConfigOptions.key("connector").stringType().noDefaultValue())).isEqualTo("bigquery");
		assertThat(options.get(ConfigOptions.key("table").stringType().noDefaultValue()))
				.isEqualTo("project.dataset.table");
		assertThat(options.get(ConfigOptions.key("filter").stringType().noDefaultValue())).isEqualTo("word_count>100");
		assertThat(options.get(ConfigOptions.key("format").stringType().noDefaultValue())).isEqualTo("arrow");
	...
	public void createDynamicTableSourceTestFailure() {
		...
		FactoryUtil.TableFactoryHelper inputHelperObject = FactoryUtil
				.createTableFactoryHelper(bigQueryDynamicTableFactory, MockDynamicTableContext);
		assertThrows("One or more required options are missing", ValidationException.class, () -> {
			inputHelperObject.validate();
		});
	}
	...
	private MockDynamicTableContext createContextObject() {
		...
		ObjectIdentifier tableIdentifier = ObjectIdentifier.of("default-catalog", "default-dataset", "flink-table");
		...
		MockDynamicTableContext contextObj = new MockDynamicTableContext(tableIdentifier, resolvedCatalogTable,
				configOptions, options, classloader, false);
		return contextObj;
	}
	...
	private MockDynamicTableContext createIncorrectContextObject() {
		...
		TableSchema tableSchema = tableSchemaProps.getOptionalTableSchema("Schema").orElseGet(() -> tableSchemaProps
				.getOptionalTableSchema("generic.table.schema").orElseGet(() -> TableSchema.builder().build()));
		Map<String, String> configOptions = new HashMap<>();
		String bigqueryReadTable = "project.dataset.table";
		configOptions.put("table", bigqueryReadTable);
		configOptions.put(FactoryUtil.FORMAT.key(), "arrow");
		...
		MockDynamicTableContext contextObj = new MockDynamicTableContext(tableIdentifier, getResolvedCatalogTable(),
				configOptions, options, classloader, false);
		return contextObj;
	}
	...
	private ResolvedCatalogTable getResolvedCatalogTable() {
		...
		configOptions.put("selectedFields", "word,word_count");
		configOptions.put("filter", "word_count>100");
		CatalogTable catalogTable = CatalogTable.of(tableSchema, "sample table creation", new ArrayList<String>(),
				configOptions);
