 ...
 * this factory creates and caches clients and also closes them during JVM
 * shutdown.
 */
public class BigQueryClientFactory implements Serializable {
	...
	@Inject
	public BigQueryClientFactory(BigQueryCredentialsSupplier bigQueryCredentialsSupplier, HeaderProvider headerProvider,
	...
	public int hashCode() {
		...
		if (credentials instanceof ExternalAccountCredentials) {
			return Objects.hashCode(BigQueryUtil.getCredentialsByteArray(credentials), headerProvider, bqConfig);
		}
	...
	public boolean equals(Object o) {
		...
		if (Objects.equal(headerProvider, that.headerProvider) && Objects
				.equal(new BigQueryClientFactoryConfig(bqConfig), new BigQueryClientFactoryConfig(that.bqConfig))) {
	...
	private BigQueryWriteClient createBigQueryWriteClient() {
		try {
			InstantiatingGrpcChannelProvider.Builder transportBuilder = BigQueryWriteSettings
					.defaultGrpcTransportProviderBuilder().setHeaderProvider(headerProvider);
			setProxyConfig(transportBuilder);
	...
	private void setProxyConfig(InstantiatingGrpcChannelProvider.Builder transportBuilder) {
		...
		if (proxyConfig.getProxyUri().isPresent()) {
			transportBuilder.setChannelConfigurator(BigQueryProxyTransporterBuilder.createGrpcChannelConfigurator(
					proxyConfig.getProxyUri(), proxyConfig.getProxyUsername(), proxyConfig.getProxyPassword()));
		}
	}
