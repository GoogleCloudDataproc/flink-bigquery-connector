...
/**
 * Instrumented {@link java.util.Enumeration} for translating bytes received
 * from the BQ Storage API to continuous input streams.
 */
public class ReadRowsResponseInputStreamEnumeration implements java.util.Enumeration<InputStream> {
	...
	public ReadRowsResponseInputStreamEnumeration(Iterator<ReadRowsResponse> responses,
	...
	void loadNextResponse() {
		...
		tracer.readRowsResponseObtained(
				currentResponse == null ? 0 : currentResponse.getArrowRecordBatch().getSerializedRecordBatch().size());
	}
}
