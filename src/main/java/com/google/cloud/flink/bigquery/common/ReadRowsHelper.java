...
public class ReadRowsHelper implements AutoCloseable {
	...
	public static final class Options implements Serializable {
		...
		public Options(int maxReadRowsRetries, Optional<String> endpoint, int backgroundParsingThreads,
	...
	public ReadRowsHelper(BigQueryClientFactory bigQueryReadClientFactory, ReadRowsRequest.Builder request,
			Options options) {
		this.bigQueryReadClientFactory = requireNonNull(bigQueryReadClientFactory,
				"bigQueryReadClientFactory cannot be null");
		this.requests = ImmutableList.of(requireNonNull(request, "request cannot be null"));
		this.options = options;
	}

	public ReadRowsHelper(BigQueryClientFactory bigQueryReadClientFactory, List<ReadRowsRequest.Builder> requests,
			Options options) {
		this.bigQueryReadClientFactory = requireNonNull(bigQueryReadClientFactory,
				"bigQueryReadClientFactory cannot be null");
	...
	public Iterator<ReadRowsResponse> readRows() {
		...
		incomingStream = new StreamCombiningIterator(client, requests, options.prebufferResponses,
				options.getMaxReadRowsRetries());
