...
public class ReadSessionCreator {
	/**
	 * Default parallelism to 1 reader per 400MB, which should be about the maximum
	 * allowed by the BigQuery Storage API. The number of partitions returned may be
	 * significantly less depending on a number of factors.
	 */
	...
	public ReadSessionCreator(ReadSessionCreatorConfig config, BigQueryClient bigQueryClient,
	...
	static int getMaxNumPartitionsRequested(OptionalInt maxParallelism, StandardTableDefinition tableDefinition) {
		return maxParallelism.orElse(Math.max((int) (tableDefinition.getNumBytes() / DEFAULT_BYTES_PER_PARTITION), 1));
	}
	 ...
	 *
	 * <p>
	 * Some attributes are governed by the {@link ReadSessionCreatorConfig} that
	...
	public ReadSessionResponse create(TableId table, ImmutableList<String> selectedFields, Optional<String> filter) {
		...
		readOptions.setArrowSerializationOptions(
				ArrowSerializationOptions.newBuilder().setBufferCompression(config.getArrowCompressionCodec()).build());

		ReadSession readSession = bigQueryReadClient.createReadSession(request.newBuilder()
				.setParent("projects/" + bigQueryClient.getProjectId())
				.setReadSession(requestedSession.setDataFormat(config.getReadDataFormat()).setReadOptions(readOptions)
						.setTable(tablePath).build())
				.setMaxStreamCount(getMaxNumPartitionsRequested(config.getMaxParallelism(), tableDefinition)).build());

		return new ReadSessionResponse(readSession, actualTable);
	}

	String toTablePath(TableId tableId) {
		return format("projects/%s/datasets/%s/tables/%s", tableId.getProject(), tableId.getDataset(),
				tableId.getTable());
	}

	TableInfo getActualTable(TableInfo table, ImmutableList<String> requiredColumns, Optional<String> filter) {
	...
	TableInfo getActualTable(TableInfo table, ImmutableList<String> requiredColumns, String[] filters) {
		...
		if (isInputTableAView(table)) {
			...
			return bigQueryClient.materializeViewToTable(querySql, table.getTableId(),
					config.getMaterializationExpirationTimeInMinutes());
		} else {
			// not regular table or a view
			throw new BigQueryConnectorException(UNSUPPORTED,
