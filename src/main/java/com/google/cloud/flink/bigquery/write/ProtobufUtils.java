...
public final class ProtobufUtils {
  ...
  public static Descriptors.Descriptor toDescriptor(RowType schema)
    ...
    while (streamdata.hasNext()) {
      ...
      if (elem.getType().getTypeRoot().toString() == "ROW") {
        ...
        listOfFileds.add(
            Field.newBuilder(
                    elem.getName(), StandardSQLTypeHandler.handle(elem.getType().toString()))
  ...
  static ArrayList<Field> getListOfSubFileds(LogicalType logicalType) {
    ...
    for (int i = 0; i < logicalTypeList.size(); i++) {
      listOfSubFileds.add(
          Field.newBuilder(
                  fieldNameList.get(i),
                  StandardSQLTypeHandler.handle(logicalTypeList.get(i).toString()))
              .setMode(Mode.NULLABLE)
              .build());
    }
  ...
  private static Object convertFlinkValueToProtoRowValue(
    ...
    if (flinkType instanceof TimeType) { // #########
      // return ((Number) flinkValue).longValue() * 1000;
