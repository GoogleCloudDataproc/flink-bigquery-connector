...
public class BigQueryClient {
	...
	private static Cache<String, TableInfo> destinationTableCache = CacheBuilder.newBuilder()
			.expireAfterWrite(15, TimeUnit.MINUTES).maximumSize(1000).build();

	private final BigQuery bigQuery;
	private final Optional<String> materializationProject;
	private final Optional<String> materializationDataset;

	public BigQueryClient(BigQuery bigQuery, Optional<String> materializationProject,
	...
	/**
	 * Creates a temporary table with a time-to-live of 1 day, and the same location
	 * as the destination table; the temporary table will have the same name as the
	 * destination table, with the current time in milliseconds appended to it;
	 * useful for holding temporary data in order to overwrite the destination
	 * table.
	 *
	 * @param destinationTableId The TableId of the eventual destination for the
	 *                           data going into the temporary table.
	...
	public Table createTempTable(TableId destinationTableId, Schema schema) {
		...
		TableId tempTableId = tempProject == null ? TableId.of(tempDataset, tableName)
		...
		TableInfo tableInfo = TableInfo.newBuilder(tempTableId, StandardTableDefinition.of(schema))
				.setExpirationTime(System.currentTimeMillis() + TimeUnit.DAYS.toMillis(1)).build();
		return bigQuery.create(tableInfo);
	}
	 ...
	 *         tracked to wait until it has finished successfully).
	 */
	public Job overwriteDestinationWithTemporary(TableId temporaryTableId, TableId destinationTableId) {
		...
		QueryJobConfiguration queryConfig = QueryJobConfiguration
				.newBuilder(sqlFromFormat(queryFormat, destinationTableId, temporaryTableId)).setUseLegacySql(false)
				.build();
	...
	public String createTablePathForBigQueryStorage(TableId tableId) {
		return String.format("projects/%s/datasets/%s/tables/%s", tableId.getProject(), tableId.getDataset(),
				tableId.getTable());
	}
	...
	public TableInfo getReadTable(ReadTableOptions options) {
		...
		throw new BigQueryConnectorException(BigQueryErrorCode.UNSUPPORTED,
				String.format("Table type '%s' of table '%s.%s' is not supported", tableType,
	...
	private void validateViewsEnabled(ReadTableOptions options) {
		if (!options.viewsEnabled()) {
			throw new BigQueryConnectorException(BigQueryErrorCode.UNSUPPORTED,
					String.format("Views are not enabled. You can enable views by setting '%s' to true. Notice"
							+ " additional cost may occur.", options.viewEnabledParamName()));
		}
	}
	...
	TableId createDestinationTable(Optional<String> referenceProject, Optional<String> referenceDataset) {
		...
		String name = String.format("_bqc_%s",
	...
	public Job createAndWaitFor(JobConfiguration jobConfiguration) {
		...
		try {
			...
			throw new BigQueryException(BaseHttpServiceException.UNKNOWN_CODE,
					String.format("Failed to run the job [%s]", job), e);
		}
	}
	...
	public long calculateTableSize(TableInfo tableInfo, Optional<String> filter) {
		try {
			...
			if (type == TableDefinition.Type.TABLE && !filter.isPresent()) {
				...
				throw new IllegalArgumentException(String.format("Unsupported table type %s for table %s", type,
	 ...
	 * @param expirationTimeInMinutes the time in minutes until the table is expired
	 *                                and auto-deleted
	...
	/**
	 * Runs the provided query on BigQuery and saves the result in a temporary
	 * table. This method is intended to be used to materialize views, so the view
	 * location (based on its TableId) is taken as a location for the temporary
	 * table, removing the need to set the materializationProject and
	 * materializationDataset properties
	 *
	 * @param querySql                the query to be run
	 * @param viewId                  the view the query came from
	 * @param expirationTimeInMinutes the time in hours until the table is expired
	 *                                and auto-deleted
	 * @return a reference to the table
	 */
	public TableInfo materializeViewToTable(String querySql, TableId viewId, int expirationTimeInMinutes) {
		TableId tableId = createDestinationTable(Optional.ofNullable(viewId.getProject()),
	...
	private TableInfo materializeTable(String querySql, TableId destinationTableId, int expirationTimeInMinutes) {
		try {
			return destinationTableCache.get(querySql,
					new DestinationTableBuilder(this, querySql, destinationTableId, expirationTimeInMinutes));
		} catch (Exception e) {
			throw new BigQueryConnectorException(BigQueryErrorCode.BIGQUERY_VIEW_DESTINATION_TABLE_CREATION_FAILED,
					String.format("Error creating destination table using the following query: [%s]", querySql), e);
		}
	}
	...
	static class DestinationTableBuilder implements Callable<TableInfo> {
		...
		DestinationTableBuilder(BigQueryClient bigQueryClient, String querySql, TableId destinationTable,
				int expirationTimeInMinutes) {
		...
		TableInfo createTableFromQuery() {
			...
			JobInfo jobInfo = JobInfo
					.of(QueryJobConfiguration.newBuilder(querySql).setDestinationTable(destinationTable).build());
			...
			long expirationTime = createdTable.getCreationTime() + TimeUnit.MINUTES.toMillis(expirationTimeInMinutes);
			Table updatedTable = bigQueryClient
					.update(createdTable.toBuilder().setExpirationTime(expirationTime).build());
			return updatedTable;
		}
		...
		Job waitForJob(Job job) {
			try {
				...
				Thread.currentThread().interrupt();
				throw new BigQueryException(BaseServiceException.UNKNOWN_CODE,
						String.format("Job %s has been interrupted", job.getJobId()), e);
			}
		}
	}
}
