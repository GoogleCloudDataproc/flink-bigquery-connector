...
/**
 * Implementation of {@link BigQueryStorageReadRowsTracer} that accumulates and
 * logs times periodically.
 */
public class LoggingBigQueryStorageReadRowsTracer implements BigQueryStorageReadRowsTracer {
	...
	private void logData() {
		log.info(
				...
				streamName, startTime, endTime == null ? "" : endTime.toString(), format(parseTime),
				difference(sparkTime, parseTime), format(serviceTime), perSecond(serviceTime, bytes),
	...
	public BigQueryStorageReadRowsTracer forkWithPrefix(String id) {
		return new LoggingBigQueryStorageReadRowsTracer("id-" + id + "-" + streamName, logIntervalPowerOf2);
	}
