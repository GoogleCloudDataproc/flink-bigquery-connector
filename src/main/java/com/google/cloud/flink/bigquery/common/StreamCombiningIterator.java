...
/**
 * An iterator that combines one or more ReadRows requests into a single
 * iterator. Ordering of messages is not guaranteed from the streams.
 *
 * <p>
 * This is adapted from ServerStream in gax. In comparison toServerStream, this
 * implementation also buffers more a configured number of responses instead of
 * a single one. For connections with high latency between client and server
 * this can processing costs. It also allows combining one or more ReadRows
 * calls into single iterator to potentially increase perceived client
 * throughput if that becomes a bottleneck for processing.
 */
public class StreamCombiningIterator implements Iterator<ReadRowsResponse> {
	...
	// This class is engineered keep responses below capacity unless a terminal
	// state has
	// been reached.
	...
	StreamCombiningIterator(BigQueryReadClient client, Collection<ReadRowsRequest.Builder> requests,
			int bufferEntriesPerStream, int numRetries) {
		this.client = client;
	...
	synchronized void stopWithError(Throwable error) {
		synchronized (lock) {
			...
			try {
				...
				Preconditions.checkState(responses.add(error), "Responses should always have capacity to add element");
			}
		}
	}
	...
	/**
	 * Consumes the next response and asynchronously request the next response from
	 * the observer.
	 *
	...
	/**
	 * Checks if the stream has been fully consumed or cancelled. This method will
	 * block until the observer enqueues another event (response or completion
	 * event). If the observer encountered an error, this method will propagate the
	 * error and put itself into an error where it will always return the same
	 * error.
	 *
	...
	public boolean hasNext() {
		...
		// Preserve async error while keeping the caller's stacktrace as a suppressed
		// exception
		...
		// This should never really happen because currently gax doesn't throw checked
		// exceptions.
		// Wrap checked exceptions. This will preserve both the caller's stacktrace and
		// the async error.
	...
	class Observer implements ResponseObserver<ReadRowsResponse> {
		...
		/**
		 * All methods accessing controller must be synchronized using controllerLock.
		 * The states of this object are: - Fresh object: null - Stream ready (receiving
		 * responses): not null - Retrying stream: null - Stream Finished: null
		 */
		...
		public void onResponse(ReadRowsResponse value) {
			...
			// Note we don't take a global lock here, so ordering of observers might be
			// different then
			// responses. This should be OK because it should balance out in the end (there
			// should
			// never be more then bufferResponses enquered from any given observer at any
			// time).
			// Ordering is important to ensure there is always an observer present for the
			// given response.
			...
			synchronized (controllerLock) {
				if (controller != null) {
					try {
						...
						// There could be edge cases here where controller is already cancelled or not
						// yet
